...
with open(output_dir_name + 'payments_output.csv', 'r') as csv_pay: #, open('xk.csv', 'w') as output_csv:#, open(stats_file_path, 'wb') as stats_file :
     ...
     n = len(payments)
     if n == 0:
          print("No payments found in payments_output.csv — skipping stats computation.")
          exit()

     last_payment_time = int(payments[n-1]['start_time'])
     remainder = last_payment_time % 31
     end_time = last_payment_time - remainder
     batch_length = end_time / 31 if end_time > 0 else 1
     ...
     for pay in payments:
          ...
          if pay_start_time < batch_length or pay_start_time >= end_time:
               continue
          b = int(pay_start_time / batch_length) - 1
          if b < 0 or b >= n_batches:
               continue
          batches['Total'][b] += 1
          if pay['is_success'] == '1':
               batches['Success'][b] += 1
     ...
     if total_succeeded == 0:
          print("⚠️ Warning: No successful payments recorded. Some stats will be set to 0.")
          total_mean_time = 0
          total_mean_route = 0
          total_mean_attempts = 0
     else:
...
for i in range(n_batches):
     ...
     if batches['Total'][i] == 0:
          batches['Success'][i] = 0.0
          batches['FailNoPath'][i] = 0.0
          batches['FailNoBalance'][i] = 0.0
          batches['FailOfflineNode'][i] = 0.0
          batches['FailTimeoutExpired'][i] = 0.0
     else:
