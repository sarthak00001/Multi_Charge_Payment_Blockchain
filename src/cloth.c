...
int main(int argc, char *argv[]) {
  ...
  // ==== DEBUG INFO ====
  printf("\n================ DEBUG PARAMETERS ================\n");
  printf("network_from_file = %d\n", net_params.network_from_file);
  printf("nodes_filename = '%s'\n", net_params.nodes_filename);
  printf("channels_filename = '%s'\n", net_params.channels_filename);
  printf("edges_filename = '%s'\n", net_params.edges_filename);
  printf("faulty_node_probability = %lf\n", net_params.faulty_node_prob);
  printf("payments_from_file = %d\n", pay_params.payments_from_file);
  printf("n_payments = %ld\n", pay_params.n_payments);
  printf("average_payment_amount = %lf\n", pay_params.average_amount);
  printf("=================================================\n\n");

  simulation = malloc(sizeof(struct simulation));
  if (simulation == NULL) {
    fprintf(stderr, "ERROR: cannot allocate memory for simulation struct\n");
    exit(-1);
  }

  simulation->random_generator = initialize_random_generator();
  if (simulation->random_generator == NULL) {
    fprintf(stderr, "ERROR: GSL random generator initialization failed\n");
    exit(-1);
  }
  ...
  if (network == NULL) {
    fprintf(stderr, "ERROR: initialize_network() returned NULL\n");
    exit(-1);
  }

  n_nodes = array_len(network->nodes);
  n_edges = array_len(network->edges);
  printf("No. Of Nodes In Network: %ld\n", n_nodes);
  printf("No. Of Edges In Network: %ld\n", n_edges);

  printf("PAYMENTS INITIALIZATION\n");
  payments = initialize_payments(pay_params, n_nodes, simulation->random_generator);
  if (payments == NULL) {
    fprintf(stderr, "ERROR: initialize_payments() returned NULL\n");
    exit(-1);
  }
  printf("Number of payments allocated: %ld\n", array_len(payments));

  // ==== Sanity checks ====
  if (n_nodes <= 0 || n_edges < 0) {
    fprintf(stderr, "ERROR: invalid network size (n_nodes=%ld, n_edges=%ld)\n", n_nodes, n_edges);
    exit(-1);
  }

  if (array_len(payments) <= 0) {
    fprintf(stderr, "WARNING: No payments to process (array_len(payments)=0)\n");
  }
