...
void initialize_dijkstra(long n_nodes, long n_edges, struct array* payments) {
  ...
  for(i=0; i<array_len(payments); i++){
    ...
    long *pid = malloc(sizeof(long));
    if(pid == NULL){
      fprintf(stderr, "ERROR: malloc failed for job pid\n");
      exit(-1);
    }
    *pid = payment->id;
    jobs = push(jobs, pid);
...
void* dijkstra_thread(void*arg) {
  ...
  while(1) {
    ...
    // data points to a malloc'd long created in initialize_dijkstra
    ...
    if (payment == NULL) {
        fprintf(stderr, "DEBUG: [Thread %p] payment_id %ld is NULL → skipping.\n",
                (void*)pthread_self(), payment_id);
        continue;
    }

    long total_nodes = array_len(thread_args->network->nodes);
    if (payment->sender >= total_nodes || payment->receiver >= total_nodes) {
        fprintf(stderr,
                "DEBUG: [Thread %p] payment_id %ld invalid nodes → sender=%" PRIu64
                ", receiver=%" PRIu64 ", total_nodes=%ld\n",
                (void*)pthread_self(), payment_id,
                payment->sender, payment->receiver, total_nodes);
        continue;
    }

    hops = dijkstra(payment->sender, payment->receiver, payment->amount,
                    thread_args->network, thread_args->current_time,
                    thread_args->data_index, &error);
